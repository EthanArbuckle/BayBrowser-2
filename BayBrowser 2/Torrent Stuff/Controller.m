...
- (id)init {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkInterfaceChanged:) name:kReachabilityChangedNotification object:_reachability];
	return self;
}

- (void)networkInterfaceChanged:(NSNotification *)notification {
	NetworkStatus status = [_reachability currentReachabilityStatus];
	[self updateNetworkSettings:status];
}

- (void)updateNetworkSettings:(NetworkStatus)status {
	
	if (status == ReachableViaWiFi) {
		if (!SETTINGS_USE_WIFI) {
			[TSMessage showNotificationWithTitle:@"Switched to Wifi! Pausing All Torrents!" type:TSMessageNotificationTypeMessage];
		...
		if (!SETTINGS_USE_NETWORK) {
			[TSMessage showNotificationWithTitle:@"Switched to Cellular! Pausing All Torrents!" type:TSMessageNotificationTypeMessage];
			for (Torrent *t in fTorrents) {
				[t stopTransfer];
			}
		}
		...
		[TSMessage showNotificationWithTitle:@"No Interent Connection! Pausing All Torrents!" type:TSMessageNotificationTypeError];
		for (Torrent *t in fTorrents) {
			[t stopTransfer];
		}
	}
